...
| `custom_module.py` | Arbitrary modules and registered model factories (FEAT-006): registers an `Encoder` factory and trains `NNModel(params=NNModelParams(net=ModelSpec("examples.encoder", 1, {"width": 16}, seed=7)))`, prints the run (descriptor instead of built-in dims), reloads it with `NNRun.load`, rebuilds the LAST checkpoint through the registry with identical predictions, and shows that an unregistered factory raises `MissingModelFactoryError`. Then wraps a keyword-only `KeywordEncoder` instance with `KeywordInputs(("features", "mask"), target="labels")`, trains it in place (`model.net is encoder`) and reloads its runtime-only (`reconstructible=False`) checkpoint into a supplied module. Offline, CPU. Unnumbered, so it is registered explicitly in `tests/test_examples_smoke.py` (end-to-end run and bounded helper `custom_module_workflow()`, `-k custom_module`). |
...
| `04_onnx_export.py` | Export a trained model to ONNX, validate via `onnx.checker`. Its bounded `registered_module_variant()` helper (also run by `main`; `tests/test_examples_smoke.py -k registered_module_variant`, CPU) exports a model built from a registered factory (`ModelSpec`) and, when `onnxruntime` is installed, checks the graph's outputs against the CPU model at `rtol=1e-4, atol=1e-5`. |
...
| `10_knowledge_distillation.py` | Hinton-style KD: pretrain a wider teacher, then distill into a much smaller student (~4% of the teacher's parameters) via `kd_train_step_factory`. Verifies the teacher's weights are frozen across the student's training. Its bounded `objective_mode()` helper (also run by `main`; `tests/test_examples_smoke.py -k objective_mode`, CPU) distills through `kd_objective` with uneven microbatches `[2, 1]` and `accumulate_grad(4)` — a window cut short by the epoch's end — and checks the single committed update against a full-batch reference step and the teacher's tensors against their snapshot. |
