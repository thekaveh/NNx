<p align="center" class="nnx-banner">
  <img src="https://raw.githubusercontent.com/thekaveh/NNx/main/docs/assets/nnx-poster.png" alt="NNx neural training banner" width="100%">
</p>
...
![NNx architecture](https://raw.githubusercontent.com/thekaveh/NNx/main/docs/assets/architecture.png)
...
- **Multi-optimizer `Trainer`** — `nnx.trainer.Trainer` parallels `NNModel.train()` for scenarios that need disjoint optimizers (GAN G/D, actor-critic). Accepts a name-keyed dict of `NNOptimParams` (or registered `NNOptimFactoryParams`); each entry's `NNParamGroupSpec` scopes the optimizer under strict-partition semantics.
- **Optimizers** — built-in SGD / Nesterov SGD / Adam / AMSGrad / **AdamW** (decoupled weight decay) via `Optims`, plus **registered optimizer factories** (`nnx.register_optimizer_factory` + `NNOptimFactoryParams(OptimizerFactorySpec(id, version, config))`) for any other `torch.optim.Optimizer`. `NNModel` and `Trainer` build both through one hook, `nnx.build_optimizer`, which checks a factory's optimizer owns exactly the resolved parameter groups; `run.yaml` stores only the factory's id/version/config, so runs reload offline and resume validates the factory identity. See [Concepts §3.1](https://github.com/thekaveh/NNx/blob/main/docs/concepts.md#31-optimizers-adamw-registered-factories-and-the-shared-build-hook).
...
- **Model surgery — Net2Net + drop + low-rank + embedding** — `nnx.surgery.{widen, deepen, drop_layer, low_rank_factorize, expand_embedding}`. `widen` and `deepen` are function-preserving Net2Net edits (Chen/Goodfellow/Shlens, ICLR 2016) on validated topologies — `widen` for a Linear inside `nn.Sequential` / `FeedFwdNN.layers` with only elementwise ops before its consumer (Softmax/LayerNorm/BatchNorm boundaries, other containers and aliased modules are rejected up front), `deepen` through ReLU — so the surged module's forward output matches the original's *before* refinement and a freshly described `NNModel` can resume training immediately without an accuracy cliff. `low_rank_factorize` is SVD truncation on a Linear (exact at max rank, Eckart-Young-bounded below it). `drop_layer` replaces a named layer with `nn.Identity`; `expand_embedding` grows an Embedding's row count and returns a frozen-mask for the original rows. Every primitive returns a fresh `nn.Module` and composes with `NNModel.train()` for the "refine after surgery" loop.
...
- **Language modeling (opt-in via `thekaveh-nnx[lm]`)** — `TransformerNN` + `NNTransformerParams` + `NNTokenizerParams` (HF Rust BPE wrapper) + `GenerativeNNModel.generate(prompt, ...)` with **KV-cache acceleration** for autoregressive decoding. A CPU regression test requires cached decoding to be at least 1.2× faster than full recomputation for its fixed 128-token workload; results on other hardware and workloads vary. Past `max_seq_len`, the cache rebuilds per step and converges to full-recompute cost. Greedy / top-k / top-p / repetition-penalty sampling is provided through a `LogitsProcessor` chain. See [docs/lm.md](https://github.com/thekaveh/NNx/blob/main/docs/lm.md) for the full walkthrough; `examples/11_tinystories_lm.py` ships an end-to-end TinyStories-class training run.
...
- **Fluent params construction** — `NNSchedulerParams.builder()`, `NNOptimParams.builder()`, `NNTransformerParams.builder()`, and `NNTrainerParams.builder()` (the composite, wraps the prior two for the multi-optim Trainer) expose variant-gated `.adam(...)` / `.sgd(...)` / `.one_cycle(...)` / etc. methods so the user can't construct an invalid kind/field combination. `LogitsChain.builder()` extends the pattern to the LM-decoding path — chain custom logit processors in any order; the Builder sorts them into NNx's canonical order (matching `generate()`'s inline-kwargs chain) before decoding runs. All Builders are purely additive; the existing direct-kwarg ctors keep working. The four params Builders branch with `.copy()` (complete or partial, no cross-talk) and rebuild from an existing value with `Builder.from_params(params)` (`build()` reproduces it exactly, same `state()` and run id) — see `examples/builder_branching.py`.
...
- **Experiment provenance** — opt-in `train(..., provenance=ExperimentManifest(...))` records the declared intent (task, labels, model descriptor, data and split identities, objective version) with a canonical SHA-256 fingerprint and a fresh attempt record per fit (status, last committed checkpoint, resume lineage), kept separate from `run.id` and the environment snapshot; `nnx.provenance.compare` diffs two plans without loading models.
- **Reproducible splits** — `nnx.data_splits.plan_split` plans group, chronological (cutoffs + gap) and stratified splits by sample id into a JSON `SplitManifest`; `NNTabularDataset(split=manifest, id_col=...)` replays it on reordered rows without `random_split`, and duplicate, missing or unexpected ids or a changed source identity fail before any loader exists. See `examples/split_replay.py`.
- **Train-only preprocessing** — `nnx.preprocessing.Standardizer` fits on an explicit training membership (never a held-out row), reuses its frozen float64 statistics under a declared column / order / dtype schema, and reloads from JSON to serve a model without refitting; `NNTabularDataset(standardize=True)` fits on the training split, and `NNDataset(train_transform=..., eval_transform=...)` gives each split its own transform over an untouched base dataset. See `examples/preprocessing_offline.py`.
- **Typed decisions** — `nnx.decisions` asks provider-neutral `Choice` / `Boolean` / `Score` questions and returns validated, labelled probabilities; `FixedHeadProvider` serves a trained classifier's head (exact label space or bijection, capabilities checked before any model call). See [docs/decisions.md](https://github.com/thekaveh/NNx/blob/main/docs/decisions.md) and `examples/decision_fixed_head.py`.
...
Python 3.10+ with PyTorch 2.4+ / torchvision 0.19+ — the oldest pair the full test suite passes on; see the [PyTorch support matrix](https://github.com/thekaveh/NNx/blob/main/docs/external-contracts.md#21-pytorch-support-matrix) for every tested torch / torchvision / Python combination and which carry real CUDA evidence. Tested on 3.10 through 3.14. Examples in [examples/](https://github.com/thekaveh/NNx/tree/main/examples) are runnable on CPU. Optional extras keep their own version constraints (`torchao` for `[quantize]` is validated only against the frozen lock, not the floor).
...
- [Typed decisions](https://github.com/thekaveh/NNx/blob/main/docs/decisions.md) — `nnx.decisions`, the inference branch: provider-neutral `Choice` / `Boolean` / `Score` questions, validated labelled probabilities, declared provider capabilities and the fixed-head adapter for trained classifiers. Read this when a caller needs a named answer with calibrated probabilities rather than positional logits.
