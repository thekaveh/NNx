...
hook family (`train_step_fn`, `eval_step_fn`, and `trainer_step_fn`), and
content-addressed persistence under `runs/<id>/`. Specialization packages
produce hook functions that callers inject into the orchestrators. The training
loop owns callback dispatch, once-per-epoch scheduler updates, phase checkpoint
cadence, and incremental `NNRun` persistence.

The diagram below shows that package topology and lifecycle together; see
[Concepts §1](concepts.md#1-architecture) for the full written breakdown.
...
`on_train_end` in reverse callback order. The Trainer path refreshes LAST after
finalization so callback mutations are present in the persisted checkpoint.
