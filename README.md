...
- **PEFT (LoRA + adapters)** — `nnx.peft.{LoRALinear, apply_lora_to, save_lora_weights, load_lora_weights, AdapterLayer}`. LoRA wraps `nn.Linear` submodules in-place with a frozen base + trainable low-rank residual (B is zero-initialized so output at step 0 equals the pretrained behavior); `apply_lora_to(module, *patterns)` does the wrap via fnmatch globs against dotted submodule names. `save_lora_weights` persists only the lora_A/B matrices (typically 1-5% of a full state_dict). `AdapterLayer` is a bottleneck residual block for users who want explicit insertion points. See `docs/concepts.md` and `examples/07_lora_finetuning.py`.
