...
- **Embeddings — contrastive trainer + FAISS export** — `nnx.embeddings.{ContrastiveTextDataset, train_contrastive, embed_texts, text_contrastive_train_step_factory, export_to_faiss, export_to_safetensors}`. Train a domain-specific text embedder from `(anchor, positive)` pairs via the existing NT-Xent machinery, then export to a FAISS index file that any RAG framework (LangChain / LlamaIndex / Haystack / raw FAISS) can consume. NNx's job ends at the FAISS index — chunking / reranking / prompt orchestration live downstream. Optional dep: `pip install "nnx-pytorch[embeddings]"` for `faiss-cpu` + `sentence-transformers`.
- **Networks** — `FeedFwdNN` (vision / tabular), `GraphConvNN` / `GraphSageNN` / `GraphAttNN` (all built on the shared `GraphNNBase` so they differ only in their PyG layer constructor), `TransformerNN` (decoder-only LM: RMSNorm + RoPE + SwiGLU + tied embeddings + KV-cache), and `ViTNN` (small ViT encoder, used as the I-JEPA backbone).
- **Language modeling (opt-in via `nnx-pytorch[lm]`)** — `TransformerNN` + `NNTransformerParams` + `NNTokenizerParams` (HF Rust BPE wrapper) + `GenerativeNNModel.generate(prompt, ...)` with **KV-cache acceleration** for autoregressive decoding (1.9× speedup on CPU at 128 tokens, larger on GPU / longer contexts) and greedy / top-k / top-p / repetition-penalty sampling via a `LogitsProcessor` chain. See [docs/lm.md](docs/lm.md) for the full walkthrough; `examples/11_tinystories_lm.py` ships an end-to-end TinyStories-class training run.
- **GGUF + Ollama export (opt-in via `nnx-pytorch[gguf-write]`)** — `nnx.interop.write_gguf(model, tokenizer, path)` writes a llama.cpp-compatible `.gguf` (fused QKV split, SwiGLU `w1`/`w3`/`w2` → `ffn_gate`/`ffn_up`/`ffn_down`, RoPE / RMSNorm metadata). `nnx.interop.export_ollama_modelfile` bundles the GGUF with a Modelfile so `ollama create -f Modelfile` registers the model locally. See [docs/gguf.md](docs/gguf.md).
- **HuggingFace Hub (opt-in via `nnx-pytorch[hub]`)** — `NNModel` mixes in `PyTorchModelHubMixin`: `save_pretrained` / `push_to_hub` / `from_pretrained`, with safetensors as an opt-in checkpoint format via `NNCheckpoint.to_file(format="safetensors")`. See [docs/hub.md](docs/hub.md).
...
- **Callbacks** — `Callback` base class with `on_{train,epoch}_{begin,end}` hooks. Stock: `EarlyStopping`, `LRMonitor`, `ModelCheckpoint` (custom-epoch tags), `TensorBoardCallback` (opt-in via `nnx-pytorch[tensorboard]`), `WandbCallback` (opt-in via `nnx-pytorch[wandb]`). Legacy `Callable[[List[IDP]], None]` is still accepted.
...
- **ONNX export** — `NNModel.to_onnx(path, example_input)` exports the network via the legacy `torch.onnx.export` (no `onnxscript` dep needed). Pass `dynamo=True` (opt-in via `nnx-pytorch[onnx-dynamo]`) to dispatch through PyTorch's newer `torch.export`-based exporter (default in torch>=2.9; supports >2 GB models via external data; generally faster).
...
```bash
pip install nnx-pytorch                        # latest release from PyPI
```
...
```bash
pip install "nnx-pytorch[tensorboard]"         # TensorBoardCallback
pip install "nnx-pytorch[wandb]"               # WandbCallback
pip install "nnx-pytorch[onnx]"                # NNModel.to_onnx validation tooling
pip install "nnx-pytorch[onnx-dynamo]"         # NNModel.to_onnx(dynamo=True) — torch.export-based exporter
pip install "nnx-pytorch[quantize]"            # nnx.quantize_int8 (torchao PTQ INT8)
pip install "nnx-pytorch[hub]"                 # safetensors checkpoints + HuggingFace Hub publish/load
pip install "nnx-pytorch[embeddings]"          # nnx.embeddings: FAISS export + sentence-transformers
pip install "nnx-pytorch[lm]"                  # TransformerNN + HF tokenizer + generate()
pip install "nnx-pytorch[docs]"                # mkdocs build (mkdocs-material + mkdocstrings)
```
...
- [GGUF & Ollama export](docs/gguf.md) — `nnx.interop.write_gguf` for llama.cpp / Ollama / LM Studio. Read this when you've trained a `TransformerNN` and want to ship it to the llama.cpp ecosystem; includes the shell-out recipe for sub-F16 quantization via `llama-quantize`.
