...
- **Training paradigms** — `nnx.paradigms.{kd, feature_kd, simclr, mixup, cutmix}_train_step_factory`. Hinton-style knowledge distillation (teacher frozen, soft+hard loss mix) **plus FitNets-style feature distillation** (`feature_kd_train_step_factory` adds an MSE term between named teacher / student intermediate-layer activations via forward hooks, mixed in with `beta`), SimCLR contrastive (NT-Xent loss exposed), Mixup batch augmentation (any shape), CutMix batch augmentation (4D images). All share an internal `_step_helpers.finalize_step` for grad-clip + NaN guard.
...
- **Training paradigms** — `nnx.paradigms.{kd, simclr, mixup, cutmix, moe}_train_step_factory` plus the `nnx.MoELinear` drop-in layer. Hinton-style knowledge distillation (teacher frozen, soft+hard loss mix), SimCLR contrastive (NT-Xent loss exposed), Mixup batch augmentation (any shape), CutMix batch augmentation (4D images), sparse top-k Mixture-of-Experts (softmax-gated routing + Switch-style load-balancing aux loss). All share an internal `_step_helpers.finalize_step` for grad-clip + NaN guard.
