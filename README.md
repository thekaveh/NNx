...
- **Model-internals viz** — `nnx.viz.summary` (Keras-style parameter table via `torchinfo`) and `nnx.viz.weight_histogram` (per-layer Plotly histogram grid). Companion to the existing `nnx.vis_utils` run-output viz; opt-in via `pip install nnx[viz]`.
