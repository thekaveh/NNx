...
- **GGUF + Ollama export (opt-in via `nnx[gguf-write]`)** — `nnx.interop.write_gguf(model, tokenizer, path)` writes a llama.cpp-compatible `.gguf` (fused QKV split, SwiGLU `w1`/`w3`/`w2` → `ffn_gate`/`ffn_up`/`ffn_down`, RoPE / RMSNorm metadata). `nnx.interop.export_ollama_modelfile` bundles the GGUF with a Modelfile so `ollama create -f Modelfile` registers the model locally. See [docs/gguf.md](docs/gguf.md).
